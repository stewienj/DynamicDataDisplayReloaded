...
namespace NewMarkersSample.Pages
{
	...
	/// </summary>
	public partial class DifferentBuildInMarkersPageLog : Page
	{
		public DifferentBuildInMarkersPageLog()
		{
			InitializeComponent();
			Loaded += OnLoaded;
			Unloaded += OnUnloaded;
		...
		private void FillCollection(int dataSourceNumber, ObservableCollectionWrapper<Point> collection, int time)
		{
			for (int i = 0; i < count; i++)
			{
				...
				collection.Add(new Point(x, Math.Log10(1+100*(0.1 * dataSourceNumber + 0.06 * (1.0+Math.Sin(10 * x + Math.Sqrt(dataSourceNumber + 1) * 0.0005 * (time - startTime)))))));
			}
