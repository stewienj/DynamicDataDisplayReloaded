...
namespace DynamicDataDisplay.SharpDX9
{
    public abstract class BaseDxTexturePrimitive<DxVertex> : BaseDxChartElement where DxVertex : struct, ID3Point
    {
        ...
        protected virtual bool UpdateVertexBufferFromGeometrySource(IEnumerable<DxVertex> newPoints)
        {
            ...
            _pointList = (newPoints ?? Enumerable.Empty<DxVertex>()).ToArray();
        ...
        public static readonly DependencyProperty SharedTextureProperty =
            DependencyProperty.Register("SharedTexture", typeof(DxSharedTexture), typeof(BaseDxTexturePrimitive<DxVertex>), new PropertyMetadata(null, (s, e) =>
            {
                if (s is BaseDxTexturePrimitive<DxVertex> primitive)
                {
