...
namespace DynamicDataDisplay.BitmapGraphs
{
    ...
    {
        private static Lazy<Heatmap> _instance = new Lazy<Heatmap>(true);

        public static Heatmap Instance => _instance.Value;
        ...
        public EventHandler<HeatmapCalculationProgressArgs> HeatmapCalculationProgress;

        public ArrayBitmapSource<uint> DrawImage(int imageWidth, int imageHeight, int spread, IEnumerable<(System.Windows.Point Point, int Count)> points, uint[] colorMap, RenderRequest renderRequest)
        {
            ...
            try
            {
                ...
                (uint[] alphaMap, float[] pixels) = CalculateParallelInterleavedVector(imageWidth, imageHeight, spread, points, renderRequest, colorkernel, alphaKernel);
        ...
        protected virtual uint[,] NormalizeImage(int imageWidth, int imageHeight, int spread, uint[] colorMap, uint[] alphaMap, float[] pixels)
        {
        ...
        /// <summary>
        /// This calculates the heapmap iterating over all the points in parallel. Has the issue of some artifacting when the points overlap.
        /// This code isn't used now, it's just here in case I think of an improvement and want a starting point that's
        /// easier to understand.
        /// As well as artifacting this version is also slower than the vectorized version.
        /// </summary>
        protected virtual (uint[] alphaMap, float[] pixels) CalculateParallel(int imageWidth, int imageHeight, int spread, IEnumerable<System.Windows.Point> points, RenderRequest renderRequest, float[] colorKernel, uint[] alphaKernel)
        {
        ...
        /// <summary>
        /// This calculates the heapmap iterating over all the points in parallel, and uses wide instructions for performance.
        /// Has the issue of some artifacting when the points overlap.
        /// This code isn't used now, it's just here in case I think of an improvement and want a starting point that's
        /// easier to understand.
        /// </summary>
        protected virtual (uint[] alphaMap, float[] pixels) CalculateParallelVector(int imageWidth, int imageHeight, int spread, IEnumerable<(System.Windows.Point Point, int Count)> points, RenderRequest renderRequest, float[] colorKernel, uint[] alphaKernel)
        {
            int kernelBreadth = spread * 2 + 1;
        ...
        /// <summary>
        /// This is the highest performing version so far, and eliminates the artifacting. The parallelization is
        /// at the render end where all the threads each iterate over all the data points, but the horizontal lines are
        /// rendered by different threads.
        /// </summary>
        protected virtual (uint[] alphaMap, float[] pixels) CalculateParallelInterleavedVector(int imageWidth, int imageHeight, int spread, IEnumerable<(System.Windows.Point Point, int Count)> points, RenderRequest renderRequest, float[] colorKernel, uint[] alphaKernel)
        {
            // Kernel size is odd numbered, like you'd do with a convolution kernel
            int kernelBreadth = spread * 2 + 1;
            ...
            int pointsProcessedCount = 0;
            int progressIncrement = Math.Max(1, pointsList.Count / 100);

            // Get the number of logical processors (vs number of cores)
            int threadCount = (Environment.ProcessorCount * 3) >> 1;
            var threadArray = Enumerable.Range(0, threadCount).ToArray();

            Parallel.ForEach(threadArray, threadNo =>
            {
                // Note the access to the pixels and alphaMap below isn't thread safe. There's
                // minor artifacting but it's not visible to the user, so I left the code like this.
                // If thread safe is required the separately compute to 4 or 8 different arrays, and
                ...
                foreach (var point in pointsList)
                {
                    ...
                    // Kernels are split by byte alignment (offset x)
                    int kernelNo = xBase % vectorLength;
                    var vectorAlphaKernel = vectorAlphaKernels[kernelNo];
                    var vectorColorKernel = vectorColorKernels[kernelNo];

                    // Rows are interleaved by thread, nead to start at zero and end on the correct row
                    var dyStart = (imageHeight - y + threadNo - imageHeight % threadCount + threadCount) % threadCount;

                    // Note dy increments in steps of threadCount
                    for (int dy = dyStart; dy < vectorKernelHeight; dy += threadCount)
                    {
                        int imageOffset = (y + dy) * bufferWidth + xBase / vectorLength;
                        int kernelOffset = dy * vectorKernelWidth;

                        for (int dx = 0; dx < vectorKernelWidth; ++dx)
                        {
                            alphaMap[imageOffset] = Vector.Max(alphaMap[imageOffset], vectorAlphaKernel[kernelOffset]);
                            pixels[imageOffset] += vectorColorKernel[kernelOffset] * point.Count;
                            // Code for visualizing which line is being rendered by which thread
                            //pixels[imageOffset] = Vector<float>.One * (threadNo + 1);
                            imageOffset++;
                            kernelOffset++;
                        }
                    }

                    // Progress updater, just do this from the first thread
                    if (threadNo == 0)
                    {
                        var progress = ++pointsProcessedCount;
                        if ((progress % progressIncrement) == 0)
