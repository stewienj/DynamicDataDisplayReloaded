...
namespace DynamicDataDisplay.Charts
{
	...
	public abstract class AxisControl<T> : AxisControlBase
	{
		...
		public IDisposable OpenUpdateRegion(bool forceUpdate)
		{
		...
		// Used to restrict the infinite oscillating/bouncing of the axis size which can happen in corner cases where the grid size changes on axis size change
		private double lastThickness = 0;
		private DateTime lastThicknessIncrease = DateTime.Now;
		...
		protected override Size MeasureOverride(Size constraint)
		{
			...
			// Restricts when we can reduce the thickness. Basically if we oscillate within 1 second then stop it.
			Func<double, double> restrict = (thickness) =>
			{
				var timeSinceIncrease = (DateTime.Now - lastThicknessIncrease);
				if (thickness > lastThickness)
				{
					lastThicknessIncrease = DateTime.Now;
				}
				if (thickness > lastThickness || timeSinceIncrease.TotalSeconds > 1.0)
				{
			...
			// Need to make sure we don't oscillate/bounce the thickness, see above "restrict"
			if (isHorizontal)
			{
				// Thickness is the Height
				result = new Size(result.Width, restrict(result.Height));
			}
			else
			{
				// Thickness is the Width
				result = new Size(restrict(result.Width), result.Height);
			}
		...
		public void UpdateUI()
		{
			...
			var tempScreenTicks = new List<MajorTickInfo<double>>(ticks.Length);
			...
			{
				T tick = tempTicks[i];
				double tickAsDouble = convertToDouble(tick);
				double screenTick = getCoordinate(createDataPoint(tickAsDouble).DataToScreen(transform));
				if (screenTick.IsFinite())
				{
					tempScreenTicks.Add(new MajorTickInfo<double>(screenTick, tickAsDouble));
					i++;
				}
				else
				{
			...
			majorScreenTicks = tempScreenTicks.ToArray();
			...
			if (drawTicks)
				DoDrawTicks(majorScreenTicks, geomGroup.Children);
			...
			DoDrawCommonLabels(majorScreenTicks);
		...
		private void DoDrawTicks(MajorTickInfo<double>[] screenTicksX, ICollection<Geometry> lines)
		{
			for (int i = 0; i < screenTicksX.Length; i++)
			{
				...
				Point p1 = createScreenPoint1(screenTicksX[i].Value);
				Point p2 = createScreenPoint2(screenTicksX[i].Value, 1);
      ...
      if (labels != null)
      {
		...
		public event EventHandler ScreenTicksChanged;
		private MajorTickInfo<double>[] majorScreenTicks;
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		[EditorBrowsable(EditorBrowsableState.Never)]
		public MajorTickInfo<double>[] MajorScreenTicks
		{
			get { return majorScreenTicks; }
		}
	...
	public sealed class SizeInfo : IComparable<SizeInfo>
	{
