...
namespace DynamicDataDisplay
{
	...
	public class Plotter2D : Plotter
	{
		...
		/// <summary>
		/// Creates a BitmapSource of the entire chart (showing whole Viewport.Domain)
		/// at specified res and dpi.
		/// </summary>
		/// <param name="pixelWidth"></param>
		/// <param name="pixelHeight"></param>
		/// <param name="dpi"></param>
		/// <returns></returns>
		public async Task<BitmapSource> ExportHighResScreenshot(int pixelWidth, int pixelHeight, int dpi = 96)
		{
			ExportMode = true;
			var prevVis = Viewport.Visible;
			Viewport.Visible = Viewport.Domain;

			Viewport.ScreenshotParameters.Width = pixelWidth;
			Viewport.ScreenshotParameters.Height = pixelHeight;
			Viewport.ScreenshotParameters.Dpi = dpi;

			var ss = await CreateScreenshot();
			ExportMode = false;
			Viewport.Visible = prevVis;
