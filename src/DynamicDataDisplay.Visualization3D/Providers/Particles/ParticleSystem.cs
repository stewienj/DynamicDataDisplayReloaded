...
namespace DynamicDataDisplay.Visualization3D.Particles
{
	...
	public abstract class ParticleSystem : DrawableComponent
	{
		...
		public override void Initialize()
		{
			...
			if (effect == null)
			{
				effect = Effect.FromStream(device, this.GetType().Assembly.GetManifestResourceStream("DynamicDataDisplay.Visualization3D.Shaders.ParticleEffect.fx"), ShaderFlags.None);
			}
			...
			texture = Texture.FromStream(device, this.GetType().Assembly.GetManifestResourceStream("DynamicDataDisplay.Visualization3D.Textures." + settings.TextureName), Usage.Dynamic, Pool.Default);
